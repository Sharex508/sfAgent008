...
import CtmWarehouse from '@salesforce/label/c.CTM_Warehouse';
...
import unitofmeasure from '@salesforce/label/c.NATT_Checkout_UnitOfMeasure';
...
const Shipmentscolumns = [
  ...
  { label: TrackingInfoLabel, fieldName: 'TrackingNumberNew',
    type: "url",
    typeAttributes:{
      label: { fieldName: 'TrackingNumber'}, target: "_blank"    
    } 
    },
...
export default class Natt_OrderDetailsComponent extends NavigationMixin (LightningElement) {  
  ...
  OrderLineDetails({error,data}) {
    ...
    if (data) {
      ...
      console.log('ORDER ITEMS====>'+JSON.stringify(this.orderLines));
      ...
      if (this.shipments) {
        this.shipments = JSON.parse(JSON.stringify(this.shipments));
        this.shipments.forEach(res => {
          if(data.TrackingNumberNew[res.NATT_ShippingCompanyName__c]){         
          res.TrackingNumberNew = data.TrackingNumberNew[res.NATT_ShippingCompanyName__c]; 
          }
          else{
            res.TrackingNumberNew = 'blank'; 
          }        
        });
      }
