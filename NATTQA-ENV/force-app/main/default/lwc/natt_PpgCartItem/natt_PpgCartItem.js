...
import { resolve } from "c/natt_cmsResourceResolver";
import {
  getLabelForOriginalPrice,
  displayOriginalPrice
} from "c/natt_cartUtils";
...
export default class Items extends NavigationMixin(LightningElement) {
  ...
  get displayItems() {
    return this._items.map((item) => {
      ...
      newItem.availableCount=0;
      if(newItem.cartItem.productDetails.productId && this.availabilityMap[newItem.cartItem.productDetails.productId]){
        newItem.availableCount =this.availabilityMap[newItem.cartItem.productDetails.productId].NATT_AvailableQuantity__c;
        if(this.availabilityMap[newItem.cartItem.productDetails.productId].NATT_AvailableQuantity__c<=0){
          newItem.availableCount=this.availabilityMap[newItem.cartItem.productDetails.productId].NATT_AvailabilityDate__c;
        }
        if(this.availabilityMap[newItem.cartItem.productDetails.productId].NATT_ProductItem__r.NATT_SignalCode__c=='NSI'){
          newItem.availableCount='NSI';         
        }
      }
      // Set whether or not the item has a core
      newItem.hasCoreItem = newItem.coreItem !== undefined;
      return newItem;
    });
  }
  ...
  handleDeleteCartItem(clickEvt) {
    ...
    const cartItemCoreId =
      clickEvt.target.parentNode?.parentNode?.parentNode?.firstChild?.lastChild
        ?.dataset?.coreId;
    this.dispatchEvent(
  ...
  handleQuantitySelectorBlur(blurEvent) {
    ...
    // Get the item ID of the product's core item if it exists
    const cartItemCoreId =
      blurEvent.target.parentNode?.parentNode?.parentNode?.firstChild?.lastChild
        ?.dataset?.coreId;
