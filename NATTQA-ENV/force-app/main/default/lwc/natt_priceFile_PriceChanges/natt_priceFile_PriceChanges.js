...
import YearLabel from '@salesforce/label/c.NATT_Price_File_Year';
import MonthsLabel from '@salesforce/label/c.NATT_Price_File_Months';
import SelectAmountofTimeSinceUpdateLabel from '@salesforce/label/c.NATT_Price_File_Select_Amount_of_Time_Since_Update';
import NoPriceChangesFoundLabel from '@salesforce/label/c.NATT_Price_File_No_Price_Changes_found';
import SelectAmountLabel from '@salesforce/label/c.NATT_Price_File_Select_Amount';
...
export default class Natt_priceFile_PriceChanges extends LightningElement {
   ...
   */
    label = {
        ...
        DownloadAsCsvLabel,
        YearLabel,
        SelectAmountofTimeSinceUpdateLabel,
        NoPriceChangesFoundLabel,
        SelectAmountLabel,
        MonthsLabel,
    ...
    @track columnsInternal = [
      ...
      //Values for "Select Amount of Time Since Update" dropdown
      get dayOptions() {
        return [
            { label: '30', value: '30' },
            { label: '60', value: '60' },
            { label: '90', value: '90' },
            { label: '6 '+MonthsLabel, value: '183' },
            { label: '1 '+YearLabel, value: '365' },
        ];
    }

    //Variables for items in table & Time Since Update dropdown
    priceFileLines;
    @track priceFileList;
    ...
    @track errorText;
    @track disableTimeDropdown = false;
    ...
    grabCustomerPriceGroup(){
        ...
        .then(result => {
            ...
            if(this.customerPriceGroup == null || this.customerPriceGroup == '' ){
                ...
                //disable Amount of Time dropdown
                this.disableTimeDropdown = true;
    ...
    loadPriceFiles() {
        console.log('this Day Value: ' + this.dayValue);
        fetchPriceFiles({customerPriceGroup:this.customerPriceGroup, limitValue: null, lastRecordId : null, lastRecordName:null, nextOrPrevious: null, dayValue : this.dayValue, advancedDate : null})
        .then(result => {
            ...
            if(preparedPricefileList.length == 0){
                this.errorText = NoPriceChangesFoundLabel + '.';
                this.disableDownloadButton = true;
            }else{
    ...
    downloadCSVFile() {   
        ...
        // CSV File Name
        downloadElement.download = 'Price Changes - ' + this.dayValue + ' days.csv';
    ...
    //triggered when a value is selected in Amount of Time dropdown
    handleDayChange(event) {
        this.priceFileList = false;
        this.dayValue = event.detail.value;
        console.log('selected day value: ' + this.dayValue);
        fetchPriceFiles({customerPriceGroup:this.customerPriceGroup, limitValue: null, lastRecordId : null, lastRecordName:null, nextOrPrevious: null, dayValue : this.dayValue, advancedDate : null})
            .then(result => {
