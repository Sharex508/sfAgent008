...
import quickProductSearch from "@salesforce/apex/NATT_B2BGetInfo.quickProductSearch";
...
//Added by Rajasekharreddy Kotella for CTM Panama Warehouse
import updateWebCart from '@salesforce/apex/NATT_B2BGetInfo.updateWebCart';
import CTM_WAREHOUSE_LABEL from '@salesforce/label/c.CTM_Warehouse_Accounts';
import updateWebcartAndBuyerGroup from '@salesforce/apex/NATT_B2BGetInfo.updateBuyerGroup';
import getCartWarehouse from '@salesforce/apex/NATT_B2BGetInfo.getCartWarehouse';




/*** Imports from NATT_PriceAvailabilityController ***/
import getAvailable from "@salesforce/apex/NATT_PriceAvailabilityController.getCtmAvailablity";
...
import natt_ctmPriceAvailabilityLineItem from "./natt_ctmPriceAvailabilityLineItem";
...
import tooltipLabel from '@salesforce/label/c.NATT_PAO_Tool_tip'

export default class Natt_ctmPriceAvailabilityOrdering extends NavigationMixin(LightningElement) {
  @wire(MessageContext)
  messageContext;
  ...
  @track cartId;
  /** @type {natt_ctmPriceAvailabilityLineItem} - Holds the Line Item that will be displayed in the Part History modal */
  @track historyLineItem;
  /*** @type {boolean} - displays show Quantity modal when true */
  @track showQuantityCheck = false;
  // Stores account and user Indo
  @track _effectiveAccountInfo;
  ...
  // Added by Rajasekharreddy Kotella for CTM Panama Warewhouse
  @track warehouseValue;
 // @track showWarehouseField =false;
  relatedEntityCartId;
  totalProductCount;
   ...
   *
   * @type {natt_ctmPriceAvailabilityLineItem[]}
   */
  @track lineItems = [
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 1),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 2),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 3),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 4),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 5)
  ];
  ...
  @track quickEntryLineItems = [
    ...
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 4),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 5),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 6),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 7),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 8),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 9),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 10),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 11),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 12),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 13),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 14),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 15),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 16),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 17),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 18),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 19),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 20),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 21),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 22),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 23),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 24),
    new natt_ctmPriceAvailabilityLineItem(null, null, null, null, 25)
  ];
  ...
  handleAddToCart() {
    ...
    if(this.getWarehouseField){
      var errorMessage
      if(! this.warehouseValue){
        errorMessage = 'Please select a warehouse for your region.';
          this.showToastWarehouse(errorMessage);
          return;
        }
      var isValidWarehouse = this.isValidRegion();
          if( ! isValidWarehouse){
            if(this.warehouseValue){
            errorMessage = this.getRegionLabel() +' Warehouse cannot be selected for your account region';
            }
            else{
    ...
    this.lineItems.forEach((lineItem) => {
      if (!lineItem.isEmpty) {
        console.log('not lineItem.hasCoreCharge:' + lineItem.hasCoreCharge );
        console.log('264::: '+this.effectiveAccountRecordType);
        products.push([lineItem.id, lineItem.quantity]);
        if (lineItem.hasCoreCharge && this.effectiveAccountRecordType != 'Internal') {
          console.log('lineItem.hasCoreCharge:' + lineItem.hasCoreCharge);
         // products.push([lineItem.id, lineItem.quantity]);
    ...
    //console.log('communityId:' + this.currentCommunityId);
  ...
  handleAddRow() {
    ...
    for (i = 0; i < 5; i++) {
      emptyLineItems.push(
        new natt_ctmPriceAvailabilityLineItem(
          null,
          null,
          null,
          null,
  ...
  async handlePartNumberBlur(event) {
    if(this.getWarehouseField && this.isCtmStorefront && !this.warehouseValue){
      var errorMessage = 'Please select a Warehouse';
      this.showToastWarehouse(errorMessage);             
      return;
    }
    ...
    if (enteredPartNumber === "") {
      this.lineItems[rowIndex] = new natt_ctmPriceAvailabilityLineItem(
        null,
        null,
        null,
        null,
  ...
  handleEnterAvailability(event) {
		try {
			...
			console.error('error in handleEnterAvailability ::', error);	
		}
  }
  ...
  handleLeaveAvailability(event) {
			try {
				...
				console.error('error in handleLeaveAvailability ::', error);	
			}
  }
  /****************************************************
   ...
   *
   *  @param {natt_ctmPriceAvailabilityLineItem[]} lineItemArray Array of line items to correct
   *  @returns {natt_ctmPriceAvailabilityLineItem[]} Original array with Row Numbers recalculatd
   */
  ...
  @wire(getUserAccount)
    wiredUserAccount({ error, data }) {
      console.log('data--'+JSON.stringify(data));
        if (data) {
            this._effectiveAccountInfo = data; 
            //this.validateUser();
        } else if (error) {
  ...
  */
  ...
  updateCartInformation() {
    ...
    getCartSummary({
      ...
      .then((result) => {
        //console.log('updateCartInfo-- '+JSON.stringify(result));
        //console.log('Related Entity ID-- '+ result.messagesSummary.relatedEntityId );
        this.relatedEntityCartId = result.messagesSummary.relatedEntityId;
        this.cartSummary = result;
        //console.log('cartSummary--'+ this.cartSummary);
        if(this.cartSummary && this.cartSummary.cartId ){
           this.getCart();
          }
          if(this.cartSummary && this.cartSummary.totalProductCount){
            let stringVal = this.cartSummary.totalProductCount;
            const total = parseInt(stringVal, 10); 
            this.totalProductCount = total;
          }
      })
      .catch((e) => {
        console.log('getCartSummary Error '+e);
        this.error = e;
      });
  }
  ...
  grabUrl() {
    getCommunityUrl()
      .then(result => {
        console.log('Solution Center Result: ' + result);
        this.solutionCenterUrl = result;
      })
      .catch(error => {
  ...
  async handleFileUpload(event) {
    if (event.target.files.length === 1) {
      ...
      if (window.FileReader) {
        ...
        this.showSpinner = false;
        /*
        this.readFileText(event.target.files[0])
          .then((csv) => {
            let uploadedRecords = this.processCsv(csv);
            let i;
            for (i = 0; i < uploadedRecords.length; i++) {               
              promises.push(this.updateProductAsync(uploadedRecords[i][0],uploadedRecords[i][1],i));
            }
            await Promise.all(promises).then(()=>{
  ...
  async updateProductAsync(partNumber, quantity, rowIndex, supercededPartNumber) {
    ...
    if (productInfo !== "undefined") {
      ...
      promises.push(getAvailable({ productId: productInfo.id, warehouse: this.warehouseValue}));	
      console.log('this.warehouseValue+>>'+this.warehouseValue);
      ...
      await Promise.all(promises).then(result => {
        ...
        //console.log('getAvailable:' + JSON.stringify(result[0]));
        availabilityResult = result[0];
        //console.log('getHistory:' + JSON.stringify(result[1]));
        historyResult = result[1];
        //console.log('getUserGuide:' + JSON.stringify(result[2]));
        userGuideResult = result[2];
        //console.log('getProductPrice:' + JSON.stringify(result[3]));
        productPrice = result[3];
      });

      console.log('after all finished');
      newLineItem = new natt_ctmPriceAvailabilityLineItem(
        productInfo,
        availabilityResult,
        historyResult,
        userGuideResult,
  ...
  async handlePartNumberBlurQuickEntry(event) {    
    ...
    if (enteredPartNumber === "") {
      this.quickEntryLineItems[rowIndex] = new natt_ctmPriceAvailabilityLineItem(
        null,
        null,
        null,
        null,
...
getWebstoreName(){
 ...
 // Added by Rajasekharreddy Kotella for CTM Panam Warehouse
 get warehouseOptions() {
  let options = [
      { label: 'Select an Option', value: '' },
      { label: 'Mexico', value: 'MEX' },
      { label: 'Challenge Miami', value: 'MIA' },
      { label: 'Manufacturing Plant', value: 'LATAM' },
 ...
 async handleChange(event){
    ...
    const priorValue = this.warehouseValue; 
    console.log('priorValue-'+ priorValue);
    if(field === 'warehouse'){
      this.warehouseValue = event.target.value;
      console.log('Warehouse value selected: '+ this.warehouseValue); 
      // this.cartObject.NATT_Warehouse__c = this.warehouseValue;
      var isValidWarehouse = this.isValidRegion();
      console.log('Selected value isValidWarehouse- '+ isValidWarehouse);
          if( ! isValidWarehouse){
            if(this.warehouseValue){
              setTimeout(() => {
                this.warehouseValue = priorValue;
              }, 1500);
              var errorMessage = this.getRegionLabel() +' Warehouse cannot be selected for your account region';
              }
              else{
                errorMessage = 'Please select a Warehouse';
              }
           ...
           await this.checkCartIsEmpty();
            if(this.totalProductCount && this.totalProductCount > 0 && priorValue != this.warehouseValue  ) {
              console.log();
              setTimeout(() => {
                //this.warehouseValue = priorValue;
                this.getCart();
              }, 1500);
            var errorMsg  = 'Place an order for Products in Cart or clear Cart to update warehouse.';
            this.showToastWarehouse(errorMsg); 
            return;
          } 
          else{ 
            this.updateWebCartHandler();
  ...
  validateLineItems() {
    console.log('Validate Items--')
    for (let i = 0; i < this.lineItems.length; i++) {
      if (this.lineItems[i].partNumber) {
        this.updateProductAsync(this.lineItems[i].partNumber, this.lineItems[i].quantity, i);
      }
    }
    }
  ...
  updateBuyerGroup() {
    ...
    updateWebcartAndBuyerGroup({ AccountID: this.resolvedEffectiveAccountId, Warehouse: this.warehouseValue})
        .then(result => {
            console.log('Buyer Group updated successfully.'+JSON.stringify(result));
        })
        .catch(error => {
            // Handle error
  ...
  /* validateUser() {
    console.log('Value--'+ this._effectiveAccountInfo.Contact.Account.NATT_Region__c);
    console.log('Profile--'+ this._effectiveAccountInfo?.Profile?.Name);
    console.log('isctm store--'+ this.isCtmStorefront);
    if(this.isCtmStorefront && this._effectiveAccountInfo && this._effectiveAccountInfo.Profile.Name == 'CTM Full Access Community' && 
      this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c &&
       ( this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c == 'Mexico' ||
         this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c == 'Latin America' )){
      this.showWarehouseField = true;
      console.log('Mx Users-');
    }
    console.log('Mx Users-out');
  } */

  isValidRegion(){
    console.log('Acc Region-'+ this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c);
    var accRegion =  this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c;
    var accName = this._effectiveAccountInfo?.Contact?.Account?.Name;
    console.log('Selected Region- '+ this.warehouseValue);
    var latinAmericaNames = [ 'PAN', 'MIA','LATAM' ];
    var mexAccWarehouse = [ 'MEX', 'LATAM', 'MIA' ];
    if (CTM_WAREHOUSE_LABEL) {
        this.ctmMexicoAcc = CTM_WAREHOUSE_LABEL.split(',');
        console.log('this.ctmMexicoAcc-'+this.ctmMexicoAcc);
        console.log('logedin  Account --'+accName +'- '+  this.ctmMexicoAcc.includes(accName) );
    }
    if(this.ctmMexicoAcc && this.ctmMexicoAcc.includes(accName) && accRegion &&
    accRegion === 'Mexico' && this.warehouseValue &&
     mexAccWarehouse.includes(this.warehouseValue)){
        return true;
    }
    else if(accRegion && accRegion === 'Mexico' && this.warehouseValue && this.warehouseValue === 'MEX' ){
        return true;
    }
    else if(accRegion && accRegion === 'Latin America' && this.warehouseValue  &&  latinAmericaNames.includes(this.warehouseValue)) {
        return true;
    }
    return false;
    
}
showToastWarehouse(errorMessage){
...
getRegionLabel(){
const selectedOption = this.warehouseOptions.find(option => option.value === this.warehouseValue);
    if (selectedOption) {
        this.warehouseLabel = selectedOption.label;
        return this.warehouseValue && this.warehouseLabel ? this.warehouseLabel : '';
    }
}

validateWarehouseRegions(){
if(this.getWarehouseField &&  !this.warehouseValue){
    const event = new ShowToastEvent({
...
updateWebCartHandler() {

  const warehouseVal = this.warehouseValue;
  console.log('WebCart update function-'+this.relatedEntityCartId );
  updateWebCart({ cartId: this.relatedEntityCartId, warehouseVal })
      .then(() => {
          console.log('WebCart updated successfully');
      })
      .catch(error => {
          console.error('Error updating WebCart:', error);
      });
  }
   ...
   checkCartIsEmpty() {
    try {
      console.log('Sync 2');
      const result =  getCartSummary({
        communityId: this.currentCommunityId,
        effectiveAccountId: this.resolvedEffectiveAccountId
      });
  
      console.log('Sync 3');
      if (result && result.totalProductCount) {
        let stringVal = result.totalProductCount;
        const total = parseInt(stringVal, 10); 
        console.log('updateCartInfo--total ' + total);
        this.totalProductCount = total;
        return total;
      } else {
        return 0; // Default value if there's no totalProductCount
      }
    } catch (e) {
      console.error('Error--', e);
      this.error = e;
      throw e; // Rethrow the error to handle it in onChange
    }
  }
  get getWarehouseField(){
    if(this.isCtmStorefront && this._effectiveAccountInfo && this._effectiveAccountInfo.Profile.Name == 'CTM Full Access Community' && 
      this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c &&
       ( this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c == 'Mexico' ||
         this._effectiveAccountInfo?.Contact?.Account?.NATT_Region__c == 'Latin America' ))
...
getCart() {
  ...
  console.log('Account Id- '+ this.resolvedEffectiveAccountId);
  getCartWarehouse({ CartID: this.cartSummary.cartId, AccountID : this.resolvedEffectiveAccountId })
  .then((result) => {
    console.log('cart warehouse result-'+result);
      this.warehouseValue = result; 
  })
  .catch((error) => {
