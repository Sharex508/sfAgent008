...
export function exportCSVFile(totalData, fileTitle){
    ...
    const result = convertToCSV(totalData);

    if(result === null) return
    
    const blob = new Blob([result])
    let formattedDate = new Date().toJSON().slice(0,10);
    const exportedFilename = fileTitle ? fileTitle+'.csv' :'CatalogDownload_' + formattedDate +'.csv'
